...
先依 Task 4.2 要求 4 停下來報告，取得使用者同意後才修改公式（非為湊數字而調整——
依據是供應商的實際計費，屬於比官方文件更直接的真實來源）。

修正內容：`_tile_based_tokens()` 的短邊縮放改為 `min(1.0, 768 / short_side)`。

修正後以相同腳本重新實測，**八筆全部通過**：

| 模型 | 尺寸 | 估算 | 實際 | 倍率 |
|------|------|-----:|-----:|-----:|
| gpt-4o-mini | 256×256 | 8,500 | 8,500 | 1.00x |
| gpt-4o-mini | 512×512 | 8,500 | 8,500 | 1.00x |
| gpt-4o-mini | 1024×1024 | 25,501 | 25,501 | 1.00x |
| gpt-4o-mini | 1024×1536 | 36,835 | 36,835 | 1.00x |
| gpt-4.1-mini | 256×256 | 104 | 103 | 1.01x |
| gpt-4.1-mini | 512×512 | 415 | 414 | 1.00x |
| gpt-4.1-mini | 1024×1024 | 1,659 | 1,658 | 1.00x |
| gpt-4.1-mini | 1024×1536 | 2,489 | 2,488 | 1.00x |

平均倍率 1.00x、最大 1.01x、最小 1.00x，**無低估**。

實測數字已寫入 `tests/rate_limiter/test_image_tokens.py` 的
`test_tile_based_matches_measured_billing`，作為迴歸測試固定住。
...
| `gpt-4o-mini` 256×256、512×512 倍率 | 1.00x | 若為 3.00x 代表短邊縮放又退回了「無條件縮放」 |
...
| 本地模型估算誤差在可接受範圍 | **無法驗證**——Ollama 不回報 usage；改由雲端實測背書 |
